...
## joint bivariate distribution of all species summed, in 2D trait space
## Input:
## - z: 2-entry vector of trait position at which we evaluate the distribution
## - n: vector of species abundances, with as many entries as species (S)
## - m: Sx2 matrix; m[i,k] is kth entry of species i's mean trait vector
## - P: Sx2x2 tensor; P[i,k,l] is (k,l)th entry of species i's covariance matrix
## Output:
## - value of joint distribution function at point z in the 2D trait space
joint_distr <- function(z, n, m, P) {
    s <- 0
    for (i in 1:length(n)) s <- s+n[i]*dmvnorm(x=z, mean=m[i,], sigma=P[i,,])
    return(s/sum(n))
}
...
get_div <- function(n, m1, m2, P11, P12, P21, P22, q, taxis) {
    ...
    ztab <- expand.grid(taxis, taxis) %>% as.matrix %>% unname
    dens <- joint_distr(ztab, n, m, P)
    dens <- dens/sum(dens)
    func_div <- ((taxis[2]-taxis[1])^2)*(sum(dens^q))^(1/(1-q))
    return(func_div)
}
...
snail <- read_csv("snaildata.csv") %>%
    ...
    mutate(zone=ifelse(vegzoneHumid==1, "humid",
                ifelse(vegzoneArid==1, "arid", "none"))) %>%
...
snail_diversity <- snail %>%
    ...
    ungroup %>%
    ## make new columns for mean trait (m1,m2) and covar matrix (P11,P12,P21,P22)
    mutate(m1=0, m2=0, P11=0, P12=0, P21=0, P22=0)

## row by row, copy fitted parameters into the columns just created above
for (i in 1:nrow(snail_diversity)) {
    snail_diversity[i,c("m1", "m2", "P11", "P12", "P21", "P22")] <-
        as.list(snail_diversity$fit[i][[1]])
}
