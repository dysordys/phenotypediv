...
extract_dynamical_variables <- function(snap) {
    ...
    L <- (-1+sqrt(1+12*(ncol(snap)-5)))/6
    n <- rep(0, S) ## species densities
    m <- matrix(0, S, L) ## trait means
    P <- array(0, c(S, L, L)) ## phenotypic covariances
    for (i in 1:S) {
        n[i] <- snap$n[i]
        m[i,] <- unlist(snap[i,(L^2+3):(L^2+L+2)])
        ## Pvec will be (G11, G12, ...) + (E11, E12, ...)
        Pvec <- unlist(snap[i,3:(L^2+2)] + snap[i,(L^2+L+5):(2*L^2+L+4)])
        P[i,,] <- matrix(Pvec, L, L, byrow=TRUE) ## reshape into matrix
...
functional_diversity <- function(snap, q, taxis) {
    ...
    func_div <- (res^L)*(sum(s^q))^(1/(1-q)) ## normalized func. div. of order q
    return(func_div)
}
