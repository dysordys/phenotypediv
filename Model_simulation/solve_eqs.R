...
eqs <- function(time, state, pars) {
    ...
    for (i in 1:S) {
        for (j in 1:S) {
            ...
            Gamma[i,j,,] <- (as.numeric(A%*%dm)%o%as.numeric(A%*%dm)-A)*alpha[i,j]
        }
    }
    alphan <- alpha%*%n
    betan <- matrix(0, S, L)
    Gamman <- array(0, c(S, L, L))
    for (k in 1:L) {
        betan[,k] <- beta[,,k]%*%n
        for (l in 1:L) Gamman[,k,l] <- Gamma[,,k,l]%*%n
    }
    ...
    for (i in 1:S) {
        dmdt[i,] <- G[i,,]%*%(p$g[i,]-betan[i,])
        dGdt[i,,] <- (G[i,,]%*%(p$Q[i,,]-Gamman[i,,])%*%G[i,,])/2
    }
    return(list(c(dndt, dmdt, dGdt)))
}
